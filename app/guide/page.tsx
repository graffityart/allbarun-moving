...
export default function GuideHub(){const breadcrumb={"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"홈",item:SITE_URL},{"@type":"ListItem",position:2,name:"이사 준비 가이드",item:`${SITE_URL}/guide`}]};const guideList={"@context":"https://schema.org","@type":"ItemList",name:"이사 준비 가이드",itemListElement:guides.map((g,index)=>({"@type":"ListItem",position:index+1,name:g.title,url:`${SITE_URL}${g.href}`}))};return <><SiteHeader/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(breadcrumb)}}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(guideList)}}/><main><section className="sub-hero"><div className="wrap narrow"><div className="breadcrumb"><a href="/">홈</a><span>›</span><strong>이사 준비 가이드</strong></div><span className="eyebrow">올바른 이사 준비</span><h1>견적을 받기 전부터<br/>이사 후 정리까지.</h1><p>이사는 업체만 정한다고 끝나지 않습니다. 날짜와 건물 조건, 계약 확인, 전입신고와 생활서비스 이전까지 필요한 내용을 단계별로 확인하세요.</p></div></section><section className="section white"><div className="wrap"><div className="guide-hub-grid">{guides.map(g=><a className="guide-hub-card" href={g.href} key={g.href}><span>{g.tag}</span><strong>{g.title}</strong><p>{g.desc}</p><b>가이드 보기 →</b></a>)}</div></div></section><section className="section"><div className="wrap"><div className="cta"><div><h2>지역과 이사 날짜가 정해졌나요?</h2><p>실제 견적에서는 층수·주차·엘리베이터·사다리차 같은 현장조건도 함께 비교하세요.</p></div><a href={ESTIMATE_INQUIRY_URL}>무료 비교견적 시작</a></div></div></section></main><SiteFooter/></>}
