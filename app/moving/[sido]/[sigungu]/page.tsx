...
import { getLocalPagePresentation } from "@/lib/local-page-presentation";
...
function localPresentationData(local:ReturnType<typeof resolveLocalGuide>){
 const areas=local.neighborhoods??[];
 const housing=areas[0]?.note??local.localIntro;
 const access=areas[1]?.note??local.localChecklist[0]??local.movingNote;
 const traffic=local.localChecklist.find(x=>/교통|정체|시간|도로|진입|차량|주차/.test(x))??areas[2]?.note??local.localChecklist[1]??local.movingNote;
 return{housing,access,traffic};
}
function localMetaDescription(regionName:string,district:string,local:ReturnType<typeof resolveLocalGuide>){
 const areas=(local.neighborhoods??[]).slice(0,2).map(x=>x.name).filter(Boolean).join("·");
 const point=local.localChecklist[0]??local.movingNote;
 const prefix=`${regionName} ${district} 포장이사`;
 const areaText=areas?`, ${areas} 생활권`:"";
 const raw=`${prefix}${areaText}. ${point}`.replace(/\s+/g," ").trim();
 return raw.length>90?`${raw.slice(0,87).replace(/[\s,·]+$/g,"")}…`:raw;
}
...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 const{sido,sigungu}=await params;const data=getDistrictRegion(sido,sigungu);if(!data)return{};
 const{region,district}=data;const meta=getRegionalMeta(region.name,district);const local=resolveLocalGuide(region.name,district);
 ...
 const primaryArea=local.neighborhoods?.[0]?.name;const secondaryArea=local.neighborhoods?.[1]?.name;const localAreas=[primaryArea,secondaryArea].filter(Boolean).join("·");
 const title=`${district} 포장이사·이사업체 | ${localAreas?`${localAreas} `:""}지역 이사정보`;
 const description=localMetaDescription(region.name,district,local);
...
export default async function DistrictPage({params}:Props){
 const{sido,sigungu}=await params;const data=getDistrictRegion(sido,sigungu);if(!data)notFound();const{region,district}=data;
 const local=resolveLocalGuide(region.name,district);const presentation=getLocalPagePresentation(region.name,district,local,localPresentationData(local));
 const adminOffice=getLocalAdminOffice(region.name,district);const heroImage=getRegionHeroImage(sido,district);const heroVideo=getRegionHeroVideo(sido,district);const faqs=getRegionFaq(region.name,district,local);const nearby=getNearbyDistricts(sido,district);
 const factors=Array.from(new Set((local.neighborhoods??[]).flatMap(i=>i.tags??[]))).slice(0,8);const primaryArea=local.neighborhoods?.[0]?.name;const secondaryArea=local.neighborhoods?.[1]?.name;
 const pageUrl=`${SITE_URL}/moving/${encodeURIComponent(sido)}/${encodeURIComponent(district)}`;const pageTitle=getRegionalMeta(region.name,district).title;
 ...
 const heroLines=presentation.heroTitle.split("\n");
...
<section className="sub-hero region-hero"><div className="wrap region-hero-grid"><div className="region-hero-copy"><div className="breadcrumb"><a href="/">홈</a><span>›</span><a href="/moving">지역별 이사</a><span>›</span><a href={`/moving/${sido}`}>{region.name}</a><span>›</span><strong>{district}</strong></div><span className="eyebrow">{presentation.heroKicker}</span><h1>{heroLines.map((line,i)=><span key={line}>{i>0?<br/>:null}{line}</span>)}</h1><p>{local.localIntro}</p><div className="hero-actions"><a className="primary-link" href={ESTIMATE_INQUIRY_URL}>{district} 무료 비교견적</a><a className="secondary-link" href="#local-calendar">{district} 손없는날·날씨</a></div></div><div className="region-hero-media"><RegionHeroImage src={heroImage} videoSrc={heroVideo} region={region.name} district={district}/></div></div></section>
<section className="section white"><div className="wrap"><span className="eyebrow">{presentation.section.eyebrow}</span><h2 className="section-title">{presentation.section.title}</h2><p className="section-desc">{presentation.section.description}</p><div className="tips-grid region-tips">{presentation.section.cards.map(card=><div className="card" key={card.title}><strong>{card.title}</strong><p>{card.body}</p></div>)}</div><div className="local-note"><strong>{presentation.section.checklistTitle}</strong><ul className="check-list">{presentation.section.checklist.map(x=><li key={x}>{x}</li>)}</ul></div></div></section>
{local.neighborhoods?.length?<section className="section local-area-section"><div className="wrap"><div className="local-area-head"><div><span className="eyebrow">{district} 생활권</span><h2 className="section-title">{presentation.areaHeading}</h2><p className="section-desc">{presentation.areaDescription}</p></div><div className="factor-pills">{factors.map(x=><span key={x}>{x}</span>)}</div></div><div className="neighborhood-grid">{local.neighborhoods.map((x,i)=><article className="neighborhood-card" key={x.name}><span>{String(i+1).padStart(2,"0")}</span><strong>{x.name}</strong><p>{x.note}</p><div className="factor-pills">{x.tags.map(t=><span key={t}>{t}</span>)}</div></article>)}</div></div></section>:null}
<section className="section region-calendar-section" id="local-calendar"><div className="wrap"><span className="eyebrow">{district} 이삿날 선택</span><h2 className="section-title">{district} 손없는날과 {region.name} 지역 날씨</h2><p className="section-desc">{primaryArea?`${primaryArea} 생활권을 비롯한 ${district} 지역은 `:""}날짜만 고르기보다 강수확률과 기온, 지역 교통 특성을 함께 확인하면 실제 이사 준비에 도움이 됩니다.</p><MovingCalendar regionName={region.name} districtName={district} lockRegion/><div className="region-source-note"><strong>안내:</strong> 손없는날은 전통적 음력 기준 참고 정보이며 날씨는 단기예보입니다. 이사 직전 최신 예보를 다시 확인하세요.</div></div></section>
<section className="section white"><div className="wrap two-col-info"><div><span className="eyebrow">{district} 견적 준비</span><h2 className="section-title">{presentation.section.checklistTitle}</h2><p>{local.movingNote}</p><ul className="check-list">{presentation.section.checklist.map(x=><li key={x}>{x}</li>)}</ul></div><div className="highlight-card"><strong>{primaryArea?`${primaryArea} 포함 ${district} 이사 팁`:`${district} 이사 팁`}</strong><p>{local.localChecklist[0]??local.movingNote}</p><a href={ESTIMATE_INQUIRY_URL}>비교견적 문의하기 →</a></div></div></section>
...
<section className="section white"><div className="wrap"><span className="eyebrow">{district} 이사 후</span><h2 className="section-title">행정·계약·생활요금 절차를 한 번에 확인하세요</h2><p className="section-desc">{primaryArea?`${primaryArea}${secondaryArea?`·${secondaryArea}`:""} 등 ${district} 현장조건을 확인했다면 `:""}전입신고·등기 확인·생활요금 이전처럼 전국 공통 절차는 아래 전문 가이드에서 별도로 확인하세요.</p><div className="guide-grid"><a className="guide-card" href="/guide/address-change"><span className="guide-step">행정</span><strong>전입신고·확정일자</strong><p>이사 후 주소 이전과 임차인이 함께 확인할 기본 절차를 정리했습니다.</p></a><a className="guide-card" href="/guide/registry"><span className="guide-step">계약</span><strong>등기사항증명서 확인</strong><p>소유자와 권리관계를 어떤 순서로 보면 되는지 확인하세요.</p></a><a className="guide-card" href="/guide/utilities"><span className="guide-step">생활</span><strong>전기·도시가스 이전</strong><p>출발지 정산과 도착지 연결을 준비하는 기본 순서를 확인하세요.</p></a><a className="guide-card" href={adminOffice.homepage} target="_blank" rel="noopener noreferrer"><span className="guide-step">관할</span><strong>{adminOffice.officeName}</strong><p>{adminOffice.address}</p><small>{adminOffice.homepageLabel} →</small></a></div><p className="section-desc" style={{marginTop:"18px"}}>{adminOffice.note}</p></div></section>
<section className="section"><div className="wrap"><span className="eyebrow">{district} 이사 유형</span><h2 className="section-title">주거 규모와 작업범위에 맞는 이사 방식</h2><div className="service-grid"><a className="card" href="/service/packing-moving"><strong>포장이사</strong><p>포장·운반·정리와 추가서비스 포함범위를 비교하세요.</p></a><a className="card" href="/service/studio-moving"><strong>원룸이사</strong><p>짐 양, 차량 크기, 계단·엘리베이터와 기사 도움범위를 확인하세요.</p></a><a className="card" href="/service/general-moving"><strong>일반이사</strong><p>직접 포장할 경우 차량과 운반인력 범위를 명확히 하세요.</p></a><a className="card" href="/service/office-moving"><strong>사무실이사</strong><p>{local.officeNote}</p></a></div><div className="local-note" style={{marginTop:"24px"}}><strong>{presentation.officeHeading}</strong><p>{local.officeNote}</p></div></div></section>
{nearby.length>0?<section className="section white nearby-section"><div className="wrap"><span className="eyebrow">주변 지역</span><h2 className="section-title">{presentation.nearbyHeading}</h2><p className="section-desc">출발지와 도착지가 다른 시·군·구라면 양쪽 지역의 차량 접근과 이사 조건을 함께 확인하세요.</p><div className="nearby-links">{nearby.map(n=><a key={n} href={`/moving/${sido}/${encodeURIComponent(n)}`}>{n} 포장이사·이사 정보 <span>→</span></a>)}</div></div></section>:null}
