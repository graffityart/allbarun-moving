...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 const description=local?.localIntro?`${local.localIntro} ${primaryArea?`${primaryArea}을 포함한 주요 생활권별 `:""}${district} 이사 견적 비교에 필요한 현장조건과 준비사항을 정리했습니다.`:meta.description;
...
<section className="section white"><div className="wrap"><span className="eyebrow">공통 이사 절차</span><h2 className="section-title">{district} 지역정보와 이사 후 행정절차</h2><p className="section-desc">{district}에서는 주거환경과 차량 접근, 교통조건을 먼저 확인하고 전입신고·등기 확인·생활요금 이전처럼 전국 공통 절차는 별도 가이드에서 이어서 확인할 수 있습니다.</p><div className="guide-grid"><a className="guide-card" href="/guide/address-change"><span className="guide-step">행정</span><strong>전입신고·확정일자</strong><p>이사 후 주소 이전과 임차인이 함께 확인할 기본 절차를 정리했습니다.</p></a><a className="guide-card" href="/guide/registry"><span className="guide-step">계약</span><strong>등기사항증명서 확인</strong><p>소유자와 권리관계를 어떤 순서로 보면 되는지 확인하세요.</p></a><a className="guide-card" href="/guide/utilities"><span className="guide-step">생활</span><strong>전기·도시가스 이전</strong><p>출발지 정산과 도착지 연결을 준비하는 기본 순서를 확인하세요.</p></a></div></div></section>
