...
export function getRegionFaq(regionName:string,district:string,local?:LocalGuide):RegionFaq[]{
  ...
  const date:RegionFaq={
    question:dateQuestions[(h+2)%dateQuestions.length],
  ...
  const localAreaFaq:RegionFaq|undefined=areaA&&areaB?{
    ...
    answer:`같은 ${district} 안에서도 ${areaA}와 ${areaB}처럼 생활권이 달라지면 주거형태, 차량 정차 위치, 도로 폭과 작업시간 조건이 달라질 수 있습니다. 주소만 전달하기보다 건물 형태와 주차 위치, 엘리베이터·계단 여부를 각각 확인하는 것이 좋습니다.`
  }:undefined;
