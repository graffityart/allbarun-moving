...
export function getRegionalOverride(region:string,district:string):LocalGuide|undefined{return overrides[region]?.[district]??getRegionalOverrideExtra(region,district)??getRegionalOverrideThird(region,district)??getRegionalOverrideExtra3(region,district)??getChungbukExtraGuide(region,district)??getGangwonExtraGuide(region,district);}
