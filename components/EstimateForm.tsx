...
const validDate = (value: string | null) => Boolean(value && /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00`)));
...
export default function EstimateForm() {
  ...
  useEffect(() => {
    ...
    const movingDate = params.get("date");
    setData((prev) => {
      const next = { ...prev };
      if (validDate(movingDate)) next.movingDate = movingDate as string;
      if (regionName) {
        const matched = regionProfiles.find((r) => r.name === regionName || r.slug === regionName);
        if (matched) {
          next.originSido = matched.slug;
          next.originDistrict = district && matched.districts.includes(district) ? district : "";
        }
      }
      return next;
    });
  ...
  return <div className="estimate-flow"><div className="estimate-progress" aria-label={`견적 신청 ${step}단계`}><div className="progress-head"><strong>{step} / 5 단계</strong><span>{stepLabels[step - 1]}</span></div><div className="progress-track"><span style={{ width: `${step * 20}%` }} /></div><div className="progress-labels">{stepLabels.map((label, index) => <button key={label} type="button" className={index + 1 === step ? "active" : index + 1 < step ? "done" : ""} onClick={() => index + 1 < step && setStep(index + 1)}>{index + 1}. {label}</button>)}</div></div><div className="estimate-step-card">
    {step === 1 && <><div className="step-heading"><span className="form-step">01</span><div><h2>어떤 이사를 준비하시나요?</h2><p>서비스 범위가 다르기 때문에 이사 유형을 먼저 정하면 견적 비교가 쉬워집니다.</p></div></div><div className="estimate-type-grid">{movingTypes.map((item) => <button type="button" key={item.value} className={data.movingType === item.value ? "estimate-type selected" : "estimate-type"} onClick={() => setField("movingType", item.value)}><strong>{item.title}</strong><span>{item.desc}</span></button>)}</div></>}
    {step === 2 && <><div className="step-heading"><span className="form-step">02</span><div><h2>어디에서 어디로 이사하시나요?</h2><p>출발지와 도착지의 주차·엘리베이터·이동거리가 견적에 영향을 줍니다.</p></div></div><div className="route-box"><div className="route-block"><span>출발지역</span><div className="field-grid"><select className="field" value={data.originSido} onChange={(e) => { setField("originSido", e.target.value); setField("originDistrict", ""); }}><option value="">시·도 선택</option>{regionProfiles.map((r) => <option key={r.slug} value={r.slug}>{r.name}</option>)}</select><select className="field" value={data.originDistrict} onChange={(e) => setField("originDistrict", e.target.value)} disabled={!data.originSido}><option value="">시·군·구 선택</option>{originDistricts.map((d) => <option key={d} value={d}>{d}</option>)}</select></div></div><div className="route-arrow">↓</div><div className="route-block"><span>도착지역</span><div className="field-grid"><select className="field" value={data.destinationSido} onChange={(e) => { setField("destinationSido", e.target.value); setField("destinationDistrict", ""); }}><option value="">시·도 선택</option>{regionProfiles.map((r) => <option key={r.slug} value={r.slug}>{r.name}</option>)}</select><select className="field" value={data.destinationDistrict} onChange={(e) => setField("destinationDistrict", e.target.value)} disabled={!data.destinationSido}><option value="">시·군·구 선택</option>{destinationDistricts.map((d) => <option key={d} value={d}>{d}</option>)}</select></div></div></div></>}
